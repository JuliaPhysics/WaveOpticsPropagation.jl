...
# ╔═╡ fd147f54-dbf9-4a2f-8c17-938566780c5c
md"# Create a beam"
...
beam = togoc(0im .+ gaussian(Float32, sz, sigma=(100, 150))) .* togoc(cis.(conv((normal(Float32, (sz), sigma=7)), 100 .* rand(Float32, (sz)))));
...
# ╔═╡ 6280a8d5-be93-483b-9b40-725f3e52424b
L ./ sz

# ╔═╡ db1724ea-96f1-445a-9cd8-e3f5dfab57f9
λ * sz[1] * z / L / sz[1] 

# ╔═╡ 2dd88724-a5eb-4ffe-97a5-30e207959220
md" The field size after propagation is going to be"

# ╔═╡ 3f791571-2a36-4479-a853-c6ed4e8a270d
md"# Forward model
As forward model, we use different defocus patterns and diffuser noise
...
begin
	diffuser = conv((normal(Float32, (sz), sigma=4)), 100 .* rand(Float32, (sz..., N)))
	diffuser_c = 0im .+ togoc(diffuser)# .* box(sz, (120, 150))
...
# ╔═╡ 5adae7a1-9efa-49d9-8889-9b6d2aa13bf0
simshow(Array(cis.(diffuser_c2[:, :, 2])))

# ╔═╡ 3fa2e0db-847b-4392-b297-52038f201f41
@bind iz2 Slider(1:size(diffuser_c, 3))

# ╔═╡ f98d20f2-70ec-490f-99a2-a3f90e115a53
simshow(Array(diffuser_c[:, :, iz2]))
...
cat(simshow(Array(rm), γ=0.1), simshow(Array(bb), γ=0.1), dims=3)[:, :, i4]
...
git-tree-sha1 = "889a19c8d0743b162e046b9b8012a3275301b48f"
