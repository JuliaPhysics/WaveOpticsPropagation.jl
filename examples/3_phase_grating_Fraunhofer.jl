...
# ╔═╡ 08526c0f-6035-473c-b043-d0cc15de91c9
# this is our main package
using WaveOpticsPropagation

# ╔═╡ 9e421e28-1528-45fb-92c4-721d433430f9
using ImageShow, ImageIO, PlutoUI, IndexFunArrays, Plots, NDTools

# ╔═╡ 38c27296-5561-4aab-800e-160d6c60d142
using FourierTools

# ╔═╡ f5833444-50ca-4b29-8249-83777ab8e90d
TableOfContents()

# ╔═╡ e6c829aa-cb51-45dd-973c-ecf0ee3a16f9
md"# Define Physical Parameter"

# ╔═╡ 5de37c42-e4f0-4832-8a07-1581e586ce66
dx = 8f-6

# ╔═╡ 0b6150c5-dc69-4349-bb7a-b85a463ff573
N = 256

# ╔═╡ c9c7a6c9-4978-4b4a-9605-c5c7594d14ca
L = N * dx

# ╔═╡ 0bfc4c9c-3774-4d1d-aea6-49b965417741
y = fftpos(L, N, CenterFT)

# ╔═╡ 48880790-c6f1-4132-86f3-31e8f43569e4
period = dx * 10

# ╔═╡ fe384fec-0164-4b86-abda-25ad88d347eb
f = 150e-3

# ╔═╡ e1756447-9df3-481f-a262-331dcb7100ea
md"# Define Grating"

# ╔═╡ 8c7b6c08-e0b3-4ee4-87ba-28384698ba02
λ = 633e-9

# ╔═╡ 54f89144-aa29-4df8-9f97-ff60811e08b7
md"# Sweep Different ϕs"

# ╔═╡ 86e6a274-fc08-4b2a-9044-913b8ed63504
ϕs = range(0, 2π, 50)

# ╔═╡ 87d2e407-7730-41dd-9355-d9f7f31a90a7
@bind ϕ Slider(ϕs, show_value=true)

# ╔═╡ dd4e9c10-29fc-43b3-9cac-157bf1e916d8
grating = ((sin.(2 .* pi .* y' ./ period) .+ 0 .* y) .> 0) .* ϕ;

# ╔═╡ 75262eea-4db4-4918-81e9-db6ee1d8e8ab
U = cis.(grating);

# ╔═╡ b678a077-fdb5-414e-bd22-5bab7dca4e0e
simshow(U)

# ╔═╡ 176f3ce7-2a77-407c-9074-1e6b4e0fa3cc
fraunhofer = Fraunhofer(U, f, λ, L)

# ╔═╡ df6d37d9-b263-412b-b218-05e0c6c9d2d5
Ip = abs2.(fraunhofer(U)) ./ sum(abs2, U);

# ╔═╡ 68c20f08-0a68-43e1-af58-a286e8f7d912
plot(fraunhofer.params.yp[:], Ip[(begin+end)÷2+1, :], xlabel="position in m", ylim=(0, 1))

# ╔═╡ 0be138af-a809-404b-bcdb-7f6cf447d8b8
plot(Ip[(begin+end)÷2+1, 100:110])

# ╔═╡ 0042e4d1-30ca-4770-b53d-d89ce3911665
Ip[(begin+end)÷2+1, 103]

# ╔═╡ cdbe2b56-3eb3-4458-b484-196fa266eb50
simshow(Ip, γ=0.1)

# ╔═╡ a48e1e12-16b0-42fd-81b4-3db0c53f8514
function sweep_ϕ(ϕs)
	fraunhofer = Fraunhofer(U, f, λ, L)

	max_val = zeros((length(ϕs,)))

	for (i,ϕ) in enumerate(ϕs)
		grating = ((sin.(2 .* pi .* y' ./ period) .+ 0 .* y) .> 0) .* ϕ
		U = cis.(grating)
		Ip = abs2.(fraunhofer(U)) ./ sum(abs2, U);
		max_val[i] = Ip[(begin+end)÷2+1, 103]
	end
	return max_val
end

# ╔═╡ bd4e5b8d-368b-479d-8b07-e34e85d1ed66
max_vals = sweep_ϕ(ϕs)

# ╔═╡ 1fd67a60-0396-4072-8f55-4b3c1e095ccc
begin
	plot(ϕs, max_vals, ylabel="normalized intensity", label="simulation")
	plot!(ϕs, 4 / π^2 .* sin(π / 2).^2 .* sin.(ϕs ./ 2).^2, label="theory")
end
...
manifest_format = "2.0"
project_hash = "14592c51d04f1bf4f54603a426f7af39eb5c7e07"
...
[[deps.WaveOpticsPropagation]]
deps = ["CUDA", "ChainRulesCore", "EllipsisNotation", "FFTW", "FourierTools", "IndexFunArrays", "NDTools", "Zygote"]
git-tree-sha1 = "aa796c8f1e8426ac7f5f540db62b50372de9e6ae"
uuid = "c4c7a1f9-3adc-4a73-843a-d378b6c86436"
version = "0.2.0"
